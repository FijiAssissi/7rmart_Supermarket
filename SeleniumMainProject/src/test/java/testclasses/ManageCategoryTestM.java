...
public class ManageCategoryTestM extends BaseM {
	...
	public void verifyManageCategory() throws IOException, AWTException {
		...
		// ManageCategoryPageM manageCategoryobj=new ManageCategoryPageM(driver);instead
		// of passing driver again, we use chaining
		manageCategory = home.manageCategoryMoreInforClickNew();
		String categoryNameFake = fakeData.getFakeCategoryNameBook();// using fakedata object to get different category
																		// names
		manageCategory.categoryNewIconClick().addCategoryName(categoryNameFake).clickOnDiscount().addCategoryImage()
				.clickOnSaveBtn();
